...
SOFTWARE_KEYMAPS = {
    ...
    # Add more software mappings here
}
...
class ClickBLE:
    ...
    def __init__(self, mac_address: Optional[str], encrypted: bool, software_type: str = 'indieVelo', verbose: bool = False):
        ...
        if self.software_type not in SOFTWARE_KEYMAPS:
            raise ValueError(f"Unsupported software type. Choose from: {', '.join(SOFTWARE_KEYMAPS.keys())}")
        self.logger.info(f"Using {self.software_type.upper()} key bindings")
        self.mac = mac_address
        if self.mac:
            self.logger.info(f'Using MAC address "{self.mac}"')
                ...
                if self.button_up_pressed != self.last_button_up_pressed:
                    ...
                    try:
                        if not self.button_up_pressed: 
                            keyboard.press_and_release(SOFTWARE_KEYMAPS[self.software_type]['up'])
                    except ImportError:
                ...
                if self.button_down_pressed != self.last_button_down_pressed:
                    ...
                    try:
                        if not self.button_down_pressed: 
                            keyboard.press_and_release(SOFTWARE_KEYMAPS[self.software_type]['down'])
                    except ImportError:
                        self.logger.error('Replicating key presses requires root privileges in Linux. Re-run this script as root to get those working')
...
click = ClickBLE(MAC, encrypted=ENCRYPTION, software_type=args.software, verbose=args.v)
